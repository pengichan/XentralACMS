...
function CreateUserModal({ onCreated, onClose }) {
  ...
  const handleSave = async () => {
    setError('');
    const required = ['userId', 'firstName', 'lastName', 'email', 'loginPassword'];
    ...
    <div style={MODAL_BACKDROP} onClick={e => e.target === e.currentTarget && onClose()}>
      <div style={MODAL_CARD}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.35rem' }}>
              ...
              <div style={{ display: 'flex', gap: '8px' }}>
                ...
                <input
                  ...
                  placeholder={countryCode === '+65' ? '8 digits' : 'Mobile number'}
                />
              </div>
            </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginTop: '0.35rem' }}>
          ...
          />
          <label htmlFor="mustChangePassword" style={{ fontSize: '0.85rem', color: '#fff', cursor: 'pointer', fontWeight: 600 }}>
...
function EditUserModal({ targetUser, isSuperAdmin, onUpdated, onClose }) {
  const initialPhone = splitPhone(targetUser.mobileNo || '');
  ...
  const [mustChangePassword, setMustChangePassword] = useState(targetUser.mustChangePassword || false);
  ...
  const set = (k) => (e) => {
    ...
    setForm(f => ({ ...f, [k]: val }));
    // Auto-check mustChangePassword when password text is modified
    if (k === 'loginPassword' && e.target.value.trim() !== '') {
  ...
  const handleSave = async () => {
    setError('');
    const required = ['userId', 'firstName', 'lastName', 'email'];
    for (const k of required) {
      if (!form[k].trim()) { setError(`${k.replace(/([A-Z])/g, ' $1')} is required`); return; }
    }
...
export default function UserManagement() {
  ...
  const fetchAccountRequests = useCallback(async () => {
    try {
      const res = await fetch(`${API}/api/account-requests`);
      if (res.ok) setAccountRequests(await res.json() || []);
    } catch (e) { console.error(e); }
  }, []);
  ...
  const handleRequestAction = async (requestId, action) => {
    ...
    try {
      const res = await fetch(`${API}/api/account-requests/${requestId}/${action}`, {
        method: 'POST'
      });
      if (res.ok) {
        ...
        alert(`Failed to ${action} request: ` + await res.text());
      }
    } catch (e) {
      alert(`Error connecting to server to ${action} request`);
      console.error(e);
    } finally {
  ...
  return (
    <div className="pam-container">
      ...
      {/* Tabs */}
      <div style={{ display: 'flex', gap: '1rem', borderBottom: '1px solid rgba(255,255,255,0.1)', marginBottom: '1.5rem', paddingBottom: '0.2rem' }}>
        <button 
          onClick={() => setActiveTab('users')} 
          style={{
            background: 'none', border: 'none', borderBottom: activeTab === 'users' ? '2px solid #4facfe' : 'none',
            color: activeTab === 'users' ? '#4facfe' : 'rgba(255,255,255,0.6)', cursor: 'pointer',
            padding: '0.5rem 1rem', fontWeight: 600, fontSize: '0.9rem', transition: 'all 0.15s'
          }}
        >
          👥 Active Users
        </button>
        <button 
          ...
          style={{
            background: 'none', border: 'none', borderBottom: activeTab === 'requests' ? '2px solid #4facfe' : 'none',
            color: activeTab === 'requests' ? '#4facfe' : 'rgba(255,255,255,0.6)', cursor: 'pointer',
            padding: '0.5rem 1rem', fontWeight: 600, fontSize: '0.9rem', transition: 'all 0.15s',
        ...
        >
          ...
          {pendingRequestsCount > 0 && (
            <span style={{
              ...
              padding: '2px 6px', borderRadius: '999px', minWidth: '16px', textAlign: 'center'
            }}>
      ...
      {activeTab === 'requests' && (
        <div className="pam-card" style={{ overflowX: 'auto' }}>
          {loading ? (
            <p style={{ padding: '2rem', opacity: 0.6 }}>Loading requests…</p>
          ) : accountRequests.length === 0 ? (
            <p style={{ padding: '2rem', opacity: 0.6 }}>No support requests submitted yet.</p>
          ) : (
            <table className="pam-table">
              <thead>
                <tr>
              ...
              <tbody>
                {accountRequests.map(req => {
                  ...
                  let statusColor = '#ef4444'; // Denied
                  let statusBg = 'rgba(239, 68, 68, 0.1)';
                  let statusBorder = 'rgba(239, 68, 68, 0.3)';
                  
                  if (isPending) {
                    statusColor = '#ffcb42'; // Pending
                    statusBg = 'rgba(255, 203, 66, 0.1)';
                    statusBorder = 'rgba(255, 203, 66, 0.3)';
                  } else if (isApproved) {
                    statusColor = '#a8ffca'; // Approved
                    statusBg = 'rgba(168, 255, 202, 0.12)';
                    statusBorder = 'rgba(168, 255, 202, 0.3)';
                  }
                  ...
                  return (
                    <tr key={req.id}>
                      <td>
                        ...
                        <div style={{ fontSize: '0.8rem', opacity: 0.6 }}>{req.email}</div>
                      </td>
                      <td style={{ fontFamily: 'monospace', fontSize: '0.85rem' }}>{req.requestedUsername || '—'}</td>
                      <td>
                        <span style={{ 
                      ...
                      <td style={{ fontSize: '0.82rem', opacity: 0.8, maxWidth: '280px', wordBreak: 'break-word' }}>
                        {req.message || <span style={{ opacity: 0.4, fontStyle: 'italic' }}>No message</span>}
                      </td>
                      <td style={{ opacity: 0.55, fontSize: '0.78rem' }}>
                        {req.createdDate ? new Date(req.createdDate).toLocaleString() : '—'}
                      </td>
                      <td>
                        <span style={{
                          ...
                          {isPending ? (
                            <>
                              <button 
                                ...
                                style={btnStyle('#a8ffca')} 
                                disabled={busy}
                              >
                                {busy ? 'Processing...' : '✔️ Approve'}
                              </button>
                              <button 
                                onClick={() => handleRequestAction(req.id, 'deny')} 
                                style={btnStyle('#ff4c4c')} 
                                disabled={busy}
                              >
                            ...
                            <span style={{ fontSize: '0.75rem', opacity: 0.35, fontStyle: 'italic' }}>Processed</span>
                          )}
                        </div>
                      </td>
                    </tr>
