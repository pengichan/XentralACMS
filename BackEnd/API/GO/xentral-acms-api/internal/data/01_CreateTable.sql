-- 01_CreateTable.sql
-- SQL Server version: create core tables for Xentral ACMS with progress prints.
...
BEGIN
  ...
  CREATE UNIQUE NONCLUSTERED INDEX uq_users_email_active ON dbo.users(email) WHERE is_deleted = 0;
  PRINT 'Created table: dbo.users and filtered unique index uq_users_email_active';
END
ELSE
...
BEGIN
  ...
  PRINT 'Created table: dbo.account_requests';
END
ELSE
  PRINT 'Table already exists: dbo.account_requests';
